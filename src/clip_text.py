...
class ClipWithMLP(nn.Module):
    def __init__(self, dropout, model_name="openai/clip-vit-large-patch14", num_classes=6, hidden_size=512):
        super(ClipWithMLP, self).__init__()
        self.clip_model = CLIPModel.from_pretrained(model_name)
        
        # MLP for combining image and text features
        self.mlp = nn.Sequential(
            nn.Linear(self.clip_model.config.projection_dim * 2, hidden_size),  # Concatenate features
            nn.ReLU(),
            nn.Dropout(dropout),
...
class EmotionDataset(Dataset):
    def __init__(self, data=None, csv_path=None, label_col=None, image_col=None, text_col=None, image_dir=None, clip_processor=None):
        if data is not None:
            self.data = data
    ...
    def __getitem__(self, idx):
        ...
        text = self.clip_processor(text=[text], return_tensors="pt")["input_ids"].squeeze(0)
...
def train_and_evaluate(config, seed=42):
    ...
    model = ClipWithMLP(config['dropout'])
