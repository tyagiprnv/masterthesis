...
class ViTWithMLP(nn.Module):
    def __init__(self, dropout, model_name="google/vit-large-patch16-224", num_classes=6, hidden_size=512):
        super(ViTWithMLP, self).__init__()
        self.vit_model = ViTForImageClassification.from_pretrained(model_name, ignore_mismatched_sizes=True)
        ...
        self.mlp = nn.Sequential(
            nn.Linear(vit_hidden_size, hidden_size),
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def forward(self, image):
        vit_features = self.vit_model.vit(image).last_hidden_state[:, 0] 
        logits = self.mlp(vit_features)
        return logits


class EmotionDataset(Dataset):
    def __init__(self, data=None, csv_path=None, label_col=None, image_col=None, image_dir=None, processor=None):
        if data is not None:
            self.data = data
...
def train_and_evaluate(config, seed=42):
    ...
    model = ViTWithMLP(config['dropout'])
...
def main():
    ...
    log_name_base = f"exp_adamw_only_vit_lr{lr}_drop{dropout}"

    configs = []
    seed = 42
    for epochs in epochs_list:
        for freeze_vit in freeze_vit_options:  
